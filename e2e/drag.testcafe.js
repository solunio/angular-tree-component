...
fixture`Drag and Drop`.page`http://localhost:4200/#/drag`.beforeEach(
  async (t) => {
...
test('should have expected children', async (t) => {
  await t
    .expect(t.ctx.root1.getNodes().count)
    .eql(2)
    .expect(t.ctx.root2.getNodes().count)
    .eql(2)
    .expect(t.ctx.child21.getNodes().count)
    .eql(0);
});
...
test.skip('should allow to drag to drop slot', async (t) => {
  await t.ctx.child1
    .dragToDropSlot(t, t.ctx.child21)
    .expect(t.ctx.root1.getNodes().count)
    .eql(1)
    .expect(t.ctx.root2.getNodes().count)
    .eql(3);
