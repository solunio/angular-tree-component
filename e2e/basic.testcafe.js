...
fixture`Basic`.page`http://localhost:4200/#/basic`.beforeEach(async (t) => {
...
test('roots with children should have an expander icon', async (t) => {
  await t
    .expect(t.ctx.root1.getExpander().exists)
    .eql(true)
    .expect(t.ctx.root2.getExpander().exists)
    .eql(true);
});

test('roots with children should not have an expander icon', async (t) => {
  await t
    .expect(t.ctx.root3.getExpander().exists)
    .eql(false)
    .expect(t.ctx.root4.getExpander().exists)
    .eql(false)
    .expect(t.ctx.root5.getExpander().exists)
    .eql(false);
});

test('roots with children should start collapsed', async (t) => {
  await t
    .expect(t.ctx.root1.getChildren().exists)
    .eql(false)
    .expect(t.ctx.root1.isExpanded())
    .eql(false);
});
...
test('should activate & deactivate nodes on click', async (t) => {
  ...
  await t
    .expect(t.ctx.root1.isActive())
    .eql(true)
    .expect(t.ctx.root2.isActive())
    .eql(false);
...
test('should focus on a node on click', async (t) => {
  await t.ctx.root1.click(t);
  await t
    .expect(t.ctx.root1.isFocused())
    .eql(true)
    .expect(t.ctx.root2.isFocused())
    .eql(false);
  await t.ctx.root2.click(t);
  await t
    .expect(t.ctx.root1.isFocused())
