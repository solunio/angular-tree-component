...
fixture`Filter`.page`http://localhost:4200/#/filter`.beforeEach(async (t) => {
...
test('should have 3 nodes', async (t) => {
  await t
    .expect(t.ctx.tree.getNodes().count)
    .eql(3)
    .expect(t.ctx.tree.getNode('Europe').isPresent())
    .ok();
});
...
fixture`Filter child nodes`.page`http://localhost:4200/#/filter`.beforeEach(
  async (t) => {
...
test('should only show child matching filter', async (t) => {
  await t
    .expect(t.ctx.usa.getNodes().count)
    .eql(1)
    .expect(t.ctx.usa.getNode('New York').isPresent())
    .ok()
    .expect(t.ctx.usa.getNode('California').isPresent())
    .notOk();
});

fixture`Clear Filter`.page`http://localhost:4200/#/filter`.beforeEach(
  async (t) => {
    t.ctx.tree = new TreeDriver('tree-root');
...
fixture`Filter by function`.page`http://localhost:4200/#/filter`.beforeEach(
  async (t) => {
