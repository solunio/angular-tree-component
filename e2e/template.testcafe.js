...
['#tree1', '#tree2', '#tree3'].forEach((treeId) => {
  fixture`Templates ${treeId}`
    .page`http://localhost:4200/#/templates`.beforeEach(async (t) => {
  ...
  test('should use the template and pass it a node var', async (t) => {
    const root1Title = t.ctx.tree.selector
      .find('.root1Class')
      .withText('root1');
  ...
  test('should use the template and pass it an index var', async (t) => {
    const root1Index = t.ctx.tree.selector
      .find('.root1ClassIndex')
      .withText('0');
...
test('should show the loading template', async (t) => {
  ...
  const loadingComponent = t.ctx.tree.selector
    .find('.root2ClassLoading')
    .withText('Loading root2...');
